...
package MaterialStream "Examples of Simulating Material Stream using Different Methods of Flash Specification"
  ...
  model TPflash "Material Stream simulation with temperature and pressure flash specifications"
    ...
    extends ChemProc.Files.ThermodynamicPackages.RaoultsLaw(Nc = 3, C = {meth, eth, wat});
  equation
    P = 101325;
    T = 351;
  ...
  model UNIQUAC "Material stream simulated with UNIQUAC property package"
    ...
    parameter data.Ethanol eth;
    parameter data.Water wat;
    extends ChemProc.Streams.MaterialStream(Nc = 2, C = {eth, wat}, Pbubl(start = 101325), Pdew(start = 101325), xvap(start = 0.68),gmadew_c(each start = 2.2), gmabubl_c(each start = 1));
    extends ChemProc.Files.ThermodynamicPackages.UNIQUAC(Nc = 2, C = {eth, wat});
  equation
    x_pc[1, :] = {0.5, 0.5};
    F_p[1] = 50;
    P = 101325;
  ...
  model NRTL "Material stream simulated with NRTL property package"
    ...
    parameter data.Ethanol eth;
    extends ChemProc.Streams.MaterialStream(Nc = 2, C = {ohex, eth}, x_pc(each start = 0.33));
    extends ChemProc.Files.ThermodynamicPackages.NRTL(Nc = 2, C = {ohex, eth});
  equation
    x_pc[1, :] = {0.5, 0.5};
    F_p[1] = 100;
    P = 101325;
  ...
  //This example can be run using the homotopy non linear solver, the results differ from those obtained using the old frontend.
    ...
    extends ChemProc.Files.ThermodynamicPackages.GraysonStreed(Nc = 4, C = {eth, acet, dich, prop}, W_c = {0.0949, 0.1841, 0.244612, 0.3125}, SP_c = {0.00297044, 0.00449341, 0.00437069, 0.00419199}, V_c = {61, 42.1382, 84.7207, 60.4292});
    extends ChemProc.Streams.MaterialStream(Nc = 4, C = {eth, acet, dich, prop});
    //Equations
  equation
    P = 101325;
    T = 210.246;
...
<ol>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.TPflash\" style=\"font-size: 12px;\">TPflash</a>: Created to simulate a material stream with Raoults Law when temperature and pressure conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.TVFflash\" style=\"font-size: 12px;\">TVFflash</a>: Created to simulate a material stream with Raoults Law when temperature and vapor phase mole fraction conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.PVFflash\" style=\"font-size: 12px;\">PVFflash</a>: Created to simulate a material stream with Raoults Law when pressure and vapor phase mole fraction conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.PHflash\" style=\"font-size: 12px;\">PHflash</a>: Created to simulate a material stream with Raoults Law when pressure and enthalpy conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.PSflash\" style=\"font-size: 12px;\">PSflash</a>: Created to simulate a material stream with Raoults Law when pressure and entropy conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.BelBubl\" style=\"font-size: 12px;\">BelBubl</a>: Created to simulate a material stream with Raoults Law when temperature and vapor phase mole fraction conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.UNIQUAC\" style=\"font-size: 12px;\">UNIQUAC</a>: Created to simulate a material stream with UNIQUAC when temperature and pressure conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.NRTL\" style=\"font-size: 12px;\">NRTL</a>: Created to simulate a material stream with NRTL when temperature and pressure conditions are known</li>
<li><a href=\"modelica://ChemProc.Examples.MaterialStream.GraysonStreed\" style=\"font-size: 12px;\">GraysonStreed</a>: Created to simulate a material stream with Grayson-Streed when temperature and pressure conditions are known. Grayson Streed parameters are also defined.</li>
</ol>
</p>
<p>&nbsp;</p>
<p>
...
<a href=\"modelica://ChemProc.Examples.CompositeMS\" style=\"font-size: 12px;\">CompositeMS</a>.
</p>
</html>"));
end MaterialStream;