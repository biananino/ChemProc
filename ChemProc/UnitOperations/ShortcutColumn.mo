...
//==============================================================================
  ...
  ChemProc.Files.Interfaces.matConn Out1(Nc = Nc) annotation(
  ...
  ChemProc.Files.Interfaces.matConn Out2(Nc = Nc) annotation(
  ...
  ChemProc.Files.Interfaces.enConn En1 annotation(
  ...
  ChemProc.Files.Interfaces.enConn En2 annotation(
...
//Rectifying and Stripping flows Calculation
  ...
  for i in 1:Nc loop
    Hvapcond_c[i] = ChemProc.Files.ThermodynamicFunctions.HVapId(C[i].SH, C[i].VapCp, C[i].HOV, C[i].Tc, Tcond);
    Hliqcond_c[i] = ChemProc.Files.ThermodynamicFunctions.HLiqId(C[i].SH, C[i].VapCp, C[i].HOV, C[i].Tc, Tcond);
  end for;
  if Ctype == "Total" then
    Hliqcond = H_p[3];
  elseif Ctype == "Partial" then
