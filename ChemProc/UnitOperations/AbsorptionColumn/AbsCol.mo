...
model AbsCol "Model of an absorption column representing fractionating towers where mixture is separated in equilibrium stages" //Model seems to be broken, doesn't run on the old frontend either.
    extends ChemProc.Files.Icons.AbsorptionColumn;
    ...
    ChemProc.Files.Interfaces.matConn In_Top(Nc = Nc) annotation(
    ...
    ChemProc.Files.Interfaces.matConn In_Bot(Nc = Nc) annotation(
    ...
    ChemProc.Files.Interfaces.matConn Out_Top(Nc = Nc) annotation(
      Placement(visible = true, transformation(origin = {100, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {250, 300}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ChemProc.Files.Interfaces.matConn Out_Bot(Nc = Nc) annotation(
      Placement(visible = true, transformation(origin = {100, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {250, -300}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    replaceable ChemProc.UnitOperations.AbsorptionColumn.AbsTray tray[2](each Nc = Nc);
  equation
//connector equation
