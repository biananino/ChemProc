...
model CSTR
  ...
  extends ChemProc.Files.Icons.CSTR;
  ...
  extends ChemProc.Files.Models.ReactionManager.KineticReaction(Nr = 1, BC_r = {1}, Coef_cr = {{-1}, {-1}, {1}}, DO_cr = {{1}, {0}, {0}}, Af_r = {0.5}, Ef_r = {0});
...
//Mole Balance
  ...
  k_r[:] = ChemProc.Files.Models.ReactionManager.Arhenious(Nr, Af_r[:], Ef_r[:], Tout);
//===============================================================================
//Calculation of Mixer Density and Volumetric Flowrate
  for i in 1:Nc loop
    rholiq_c[i] = ChemProc.Files.ThermodynamicFunctions.Dens(C[i].LiqDen, C[i].Tc, Tout, Pout);
  end for;
