within ChemProc.UnitOperations.DistillationColumn;
  ...
  model DistCol "Model of a distillation column representing fractionating towers where mixture is separated in equilibrium stages"
    extends ChemProc.Files.Icons.DistillationColumn;
    ...
    parameter Boolean Bin_t[Nt] = ChemProc.Files.OtherFunctions.colBoolCalc(Nt, Ni, InT_s) "Stream stage associations" annotation(
    Dialog(tab = "Column Specifications", group = "Component Parameters"));
    ...
    Real RR(min = 0);
    ChemProc.Files.Interfaces.matConn In_s[Ni](each Nc = Nc) annotation(
    ...
    ChemProc.Files.Interfaces.matConn Dist(Nc = Nc) annotation(
    ...
    ChemProc.Files.Interfaces.matConn Bot(Nc = Nc) annotation(
    ...
    ChemProc.Files.Interfaces.enConn Cduty annotation(
    ...
    ChemProc.Files.Interfaces.enConn Rduty annotation(
    ...
    ChemProc.Files.Interfaces.matConn Out_s[Nout](each Nc = Nc) annotation(
    ...
    //ChemProc.Files.Interfaces.enConn En[NQ](each Nc = Nc) annotation(
    ...
    replaceable ChemProc.UnitOperations.DistillationColumn.Cond condenser(Nc = Nc, C = C, Ctype = Ctype, Bin = Bin_t[1]);
    replaceable ChemProc.UnitOperations.DistillationColumn.Reb  reboiler(Nc = Nc, C = C, Bin = Bin_t[Nt]);
    parameter Boolean Bin_tray[Nt-2] = Bin_t[2:Nt-1]; //Temporary workaround until bug is fixed
    replaceable ChemProc.UnitOperations.DistillationColumn.DistTray tray[2](each Nc = Nc, each C = C, Bin = Bin_tray);
